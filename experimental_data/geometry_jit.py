...
@numba.njit(cache=True)
def magnetic_interaction_from_dipole_moment(dipole_moment: np.ndarray) -> np.ndarray: # TODO: integrate it properly into Neelaksh code
    """
    Args:
...
def jacobian_torqueforce_to_torque(beta, alpha, l_mag):
        ...
        force_torque_relation = np.array([[0,-l_mag*np.cos(beta)*np.cos(alpha) ,-np.sin(beta)*l_mag],[np.cos(beta)*np.cos(alpha)*l_mag, 0,-np.cos(beta)*np.sin(alpha)*l_mag],[np.sin(beta)*l_mag,l_mag*np.cos(beta)*np.sin(alpha) ,0]])
...
@numba.njit(cache=True)
def RT(a, b):
    # corresponds to RT according to my default definition
    # alpha rot around y axis
    # beta rot around x axis
    #         tau_inertial = R_transposed * tau_body_fixed_frame
    # accepts symbolic or numerical values
